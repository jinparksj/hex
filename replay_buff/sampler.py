...
def rollout(env, policy, path_length, render=False, speedup=None):
    Da = env.spec.action_flat_dim
    Do = env.spec.observation_flat_dim
...
class SimpleSampler(Sampler):
    ...
    def log_diagnostics(self):
        ...
        sql_logger.record_tabular('max-path-return', self._max_path_return)
        sql_logger.record_tabular('last-path-return', self._last_path_return)
        sql_logger.record_tabular('episodes', self._n_episodes)
        sql_logger.record_tabular('total-samples', self._total_samples)
