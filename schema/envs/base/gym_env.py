...
import os
from schema.envs.base.env_base import Env, Step
...
def convert_gym_space(space):
    if isinstance(space, gym.spaces.Box):
        return Box(low=space.low, high=space.high, dtype='float32')  # float32 added
...
class GymEnv(Env, Serializable):
    ...
    def render(self, mode='human', close=False):
        return self.env.render(mode=mode)  # _render(mode, close)
        # self.env.render()

    def terminate(self):
        if self.monitoring:
            self.env.close()  # _close
    ...
    ***************************
    ...
    python launch_exp/submit_gym.py %s
