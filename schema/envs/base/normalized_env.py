...
from schema.envs.base.env_base import ProxyEnv
from gym.spaces import Box
from schema.misc.overrides import overrides
from schema.envs.base.env_base import Step
...
class NormalizedEnv(ProxyEnv, Serializable):
    ...
    def action_space(self):
        if isinstance(self._wrapped_env.action_space, Box):
            ...
            return Box(-1 * ub, ub, dtype='float32')  # added dtype
    ...
    def step(self, action):
        if isinstance(self._wrapped_env.action_space, Box):
            # rescale the action
            lb = self._wrapped_env.action_space.low.min()
            ub = self._wrapped_env.action_space.high.max()
