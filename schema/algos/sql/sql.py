...
from schema.sampler.sampler import rollouts
import tensorflow as tf
import gtimer as gt
import numpy as np
from schema.logger import logger
from schema.utils import tf_utils
from schema.utils.utils import deep_clone
...
class SQLAlgorithm(object):
    ...
    def _create_td_update(self):
        ...
        with tf.variable_scope('target'):
            ...
            tf_utils.assert_shape(q_value_targets, [None, self._value_n_particles])

        self._q_values = self.qf.output_for(
            self._observations_ph, self._actions_pl, reuse=True)
        tf_utils.assert_shape(self._q_values, [None])

        # Equation 10:
        next_value = tf.reduce_logsumexp(q_value_targets, axis=1)
        tf_utils.assert_shape(next_value, [None])
    ...
    def _create_svgd_update(self):
        ...
        tf_utils.assert_shape(fixed_actions, [None, n_fixed_actions, self._action_dim])
        tf_utils.assert_shape(updated_actions,
        ...
        tf_utils.assert_shape(grad_log_p, [None, n_fixed_actions, 1, self._action_dim])
        ...
        tf_utils.assert_shape(kappa, [None, n_fixed_actions, n_updated_actions, 1])
