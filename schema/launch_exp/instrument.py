...
from schema.misc.serializable import Serializable
from schema.launch_exp.config import config
from schema.misc.console import mkdir_p
...
from schema.launch_exp.variant import AttrDict
import collections
...
def run_experiment_lite(
        ...
        exp_name=None,
        log_dir=None,
        script="launch_exp/run_experiment_lite.py",
    ...
    the end of the experiment)
    :param sync_s3_log: Whether to sync logger files during execution of the experiment (they will always be synced at
    the end of the experiment)
...
def to_local_command(params, python_command="python", script=osp.join(config.PROJECT_PATH,
                                                                      'launch_exp/run_experiment_lite.py'),
                     use_gpu=False):
...
def to_docker_command(params, docker_image, python_command="python", script='launch_exp/run_experiment_lite.py',
...
def launch_ec2(params_list, exp_prefix, docker_image, code_full_path,
    ...
    for params in params_list:
        ...
        if periodic_sync:
            ...
            include_log = " --include '*.logger' " if sync_s3_log else " "
            # sio.write("""
            #     while /bin/true; do
            ...
            if sync_log_on_termination:
                ...
                #             aws s3 cp /home/ubuntu/user_data.logger {remote_log_dir}/stdout.logger --region {aws_region}
                ...
                #             sleep 5
                #         fi
                #     done & echo logger sync initiated
                ...
                sio.write("""
                    while /bin/true; do
                        if [ -z $(curl -Is http://169.254.169.254/latest/meta-data/spot/termination-time | head -1 | grep 404 | cut -d \  -f 2) ]
                          then
                            logger "Running shutdown hook."
                            aws s3 cp /home/ubuntu/user_data.logger {remote_log_dir}/stdout.logger
    ...
    if terminate_machine:
        ...
        """.format(aws_region=config.AWS_REGION_NAME))
    sio.write("} >> /home/ubuntu/user_data.logger 2>&1\n")
