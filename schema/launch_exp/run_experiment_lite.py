...
from schema.misc.ext import is_iterable, set_seed
from schema.launch_exp.instrument import concretize
from schema.launch_exp.config import config
import schema.logger.logger as logger
import argparse
...
def run_experiment(argv):
    ...
    parser.add_argument('--log_dir', type=str, default=None,
                        help='Path to save the logger and iteration snapshot.')
    ...
    parser.add_argument('--tabular_log_file', type=str, default='progress.csv',
                        help='Name of the tabular logger file (in csv).')
    parser.add_argument('--text_log_file', type=str, default='debug.logger',
                        help='Name of the text logger file (in pure text).')
    parser.add_argument('--params_log_file', type=str, default='params.json',
                        help='Name of the parameter logger file (in json).')
    parser.add_argument('--variant_log_file', type=str, default='variant.json',
                        help='Name of the variant logger file (in json).')
    ...
    parser.add_argument('--log_tabular_only', type=ast.literal_eval, default=False,
                        help='Whether to only print the tabular logger information (in a horizontal format)')
    ...
    if args.n_parallel > 0:
        from schema.sampler import parallel_sampler
        parallel_sampler.initialize(n_parallel=args.n_parallel)
        if args.seed is not None:
            parallel_sampler.set_seed(args.seed)

    if args.plot:
        from schema.plotter import plotter
        plotter.init_worker()
